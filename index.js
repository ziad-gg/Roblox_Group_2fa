...
class AsyncPayoutManager {
    ...
    static async PayoutVerify(userId, ChallengeId, verificationCode, type = "email") {
        return new Promise((ProcessingInvokeingFunc, PromiseRejectionFunc) => {
            ...
            }).then(({ data }) => ProcessingInvokeingFunc(data.verificationToken))
        })
    }
...
async function Payout(groupId, username, amount) {
    ...
    await instance.post(Routes.groups.payouts(groupId), {
        ...
        console.log("Payout was done without verification.")
    }).catch(async e => {
        const rblxChallengeId = e.response.headers['rblx-challenge-id'];
        ...
        const verificationToken = await PayoutVerify(data.challengeId, username);
        const metadata = JSON.stringify({ verificationToken, rememberDevice: false, challengeId: data.challengeId })
        await AsyncPayoutManager.Continue(rblxChallengeId, metadata);

        await Payout(groupId, username, amount);
        console.log("done 2fa")
    });
}
 ...
 * @returns {Promise<string>}
 */
async function PayoutVerify(ChallengeId, username) {
    const userId = await noblox.getIdFromUsername(username);
    ...
    if (verificationCode) {
        ...
        const verificationToken = await AsyncPayoutManager.PayoutVerify(userId, ChallengeId, verificationCode, type);
